...
class yolov5():
    def __init__(self, yolo_type, confThreshold=0.5, nmsThreshold=0.5, objThreshold=0.5):
        ...
        self.net = cv2.dnn.readNet('/home/hoangnt107/PycharmProjects/3CodeReg/Landmark_detection/best.onnx')
    ...
    def postprocess(self, frame, outs):
        ...
        # confidences = [[] for i in range(self.nc)]
        # boxes = [[] for i in range(self.nc)]
        # landmarks = [[] for i in range(self.nc)]
        confidences = []
        boxes = []
        landmarks = []
        classIds = []
        for detection in outs:
            ...
            if detection[4] > self.objThreshold:
                ...
                landmark = detection[5:13] * np.tile(np.float32([ratiow, ratioh]), 4)
                # print(landmark)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--yolo_type', type=str, default='yolov5s', choices=['yolov5s', 'yolov5m', 'yolov5l'],
                        help="yolo type")
    parser.add_argument("--imgpath", type=str, default='/home/hoangnt107/Downloads/dataset_barcode_landmark_v2.0/1655060795851_B0-B3-53-61-21-7C__1598768689_0.jpg', help="image path")
    parser.add_argument('--confThreshold', default=0.3, type=float, help='class confidence')
    parser.add_argument('--nmsThreshold', default=0.5, type=float, help='nms iou thresh')
    parser.add_argument('--objThreshold', default=0.3, type=float, help='object confidence')
    args = parser.parse_args()

    yolonet = yolov5(args.yolo_type, confThreshold=args.confThreshold, nmsThreshold=args.nmsThreshold,
                     objThreshold=args.objThreshold)
    srcimg = cv2.imread(args.imgpath)
    dets = yolonet.detect(srcimg)
    srcimg = yolonet.postprocess(srcimg, dets)
    cv2.imwrite('ABC.jpg', srcimg)
